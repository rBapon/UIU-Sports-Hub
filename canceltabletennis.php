...
if (isset($_POST["cancel_booking"])) {
    ...
    // Check if booking exists for the provided user ID and table tennis item
    $sqlCheckBooking = "SELECT quantity FROM booking_list WHERE item = 'Table Tennis' AND id = ?";
    ...
    if (mysqli_stmt_num_rows($stmtCheckBooking) > 0) {
        ...
        $sqlDeleteBooking = "DELETE FROM booking_list WHERE item = 'Table Tennis' AND id = ?";
        ...
        $sqlUpdateInventory = "UPDATE inventory SET table_tennis = table_tennis + ?";
...
<body>
    <div class="container">
        <h2>Cancel Table Tennis Booking</h2>
