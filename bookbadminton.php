...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($quantity <= $availableInventory) {
        ...
        if (mysqli_stmt_execute($stmtInsertBooking)) {
            ...
            $sqlUpdateInventory = "UPDATE inventory SET badminton = ?";
...
@media (max-width: 768px) {
    ...
    <script>
        window.addEventListener('beforeunload', function() {
            // This function will be triggered when the user closes or reloads the page
            location.reload();
        });
    </script>
</head>
<body>
    ...
    ?>
    <!-- Booking form -->
    <form method="post" action="bookbadminton.php">
